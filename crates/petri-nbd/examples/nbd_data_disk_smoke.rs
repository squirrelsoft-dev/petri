//! Smoke test: boot a real EFI VM from a nocloud image with a blank scratch
//! attached over NBD as a `--data-disk`, and confirm the data disk attaches and
//! the NBD client connects. No guest agent (petri-guest) is involved — this is
//! the bootstrap builder's disk topology, minus provisioning.
//!
//! Needs a built+signed `petri-vz` (with the `com.apple.security.network.client`
//! entitlement) and a Debian nocloud arm64 raw image.
//!
//! Usage (from the repo root):
//!   cargo run -p petri-nbd --example nbd_data_disk_smoke -- <nocloud.raw> [SECS]
...
fn run() -> Result<(), String> {
    ...
    // Blank writable scratch served over loopback NBD — the builder's data disk.
    ...
    if console_text.is_empty() {
        println!("  <empty — guest console not on virtio hvc0>");
    } else {
        print!("{console_text}");
    }
    println!("=== end console ===");
    ...
    if nbd_connected {
        ...
        Err("NBD data disk did not connect (see helper log above)".into())
    }
}
